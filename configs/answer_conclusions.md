...
Nine questions, nine outcomes matching what was written down before the
run. Six answered, every one of them citing chunks it was given and
inventing none. Three refused, by two different mechanisms.
...
### One answer exists because of HW3

`hw4_q5` asks how to validate and constrain LLM output. The only section in
this corpus that answers it is "2. Define and validate expected output
formats", labelled grade 2 in `configs/eval_queries.yaml`. HW2 retrieval
never returned it in the top five for that query. The BM25 half added in
HW3 brought it to rank 3, and the answer is built on it.

Retrieval work that changed no headline metric in HW3 is what makes this
